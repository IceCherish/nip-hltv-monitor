...
GitHub Actions 中有两个独立工作流：“NIP 手动测试”用于你点击运行，“NIP 自动监控（外部定时）”由外部免费定时器通过 GitHub Workflow Dispatch API 每约 6 分钟触发。外部定时任务不会被当成一次“程序启动”，所以不会每 6 分钟重复发送赛程；它会检查新新闻、转会动态和赛前提醒，并在北京时间 10 点后检查当天是否已经发过赛程，没发才补发。赛程或赛果变化本身不触发推送。你在“NIP 手动测试”中手动运行一次 `monitor`，则视为你主动启动一次，会发送一条完整的 NIP 赛程消息并记为当天已发。
