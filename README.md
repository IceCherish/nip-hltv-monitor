...
GitHub Actions 中有两个独立工作流：“NIP 手动测试”用于你点击运行，“NIP 自动监控（每6分钟）”只负责定时运行。自动工作流默认在每小时第 3、9、15、21、27、33、39、45、51、57 分钟启动临时检查任务，也就是每约 6 分钟一次并避开整点拥堵。自动任务在 Actions 列表中显示为“自动检查（每6分钟）”。定时任务不会被当成一次“程序启动”，所以不会每 6 分钟重复发送赛程；它会检查新新闻、转会动态和赛前提醒，并在北京时间 10 点后检查当天是否已经发过赛程，没发才补发。赛程或赛果变化本身不触发推送。你在“NIP 手动测试”中手动运行一次 `monitor`，则视为你主动启动一次，会发送一条完整的 NIP 赛程消息并记为当天已发。GitHub 的定时任务可能排队，因此不保证精确到秒。
